...
def _load_pil_img_as_tensor(img_pil, image_size):
    img_np = np.array(img_pil.convert("RGB").resize((image_size, image_size)))
...
def load_video_frames(
    ...
    if isinstance(video_path, str) and os.path.isdir(video_path):
        ...
        # 逐帧读取视频
        for n in tqdm(range(num_frames), desc=f"frame loading {video_path}"):
            ret, frame = cap.read()
            if not ret:
                break
            ...
            # 处理图像并获取帧的尺寸
            images[n], video_height, video_width = _load_pil_img_as_tensor(img_pil, image_size)
        cap.release()
