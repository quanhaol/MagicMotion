...
class MagicMotionPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        transformer: MagicMotionTransformer3DModel,
    ...
    def prepare_trajectory_latents(
        ...
        if isinstance(generator, list):
            trajectory_latents = [
                retrieve_latents(
                    self.vae.encode(trajectory_maps[i].unsqueeze(0)), generator[i]
                )
                for i in range(batch_size)
            ]
        else:
            trajectory_latents = [
                retrieve_latents(self.vae.encode(img.unsqueeze(0)), generator)
                for img in trajectory_maps
            ]

        trajectory_latents = (
            torch.cat(trajectory_latents, dim=0).to(dtype).permute(0, 2, 1, 3, 4)
        )  # [B, F, C, H, W]
        ...
        # Select the first frame along the second dimension
        if self.transformer.config.patch_size_t is not None:
            first_frame = trajectory_latents[
                :,
                : trajectory_latents.size(1) % self.transformer.config.patch_size_t,
                ...,
            ]
    ...
    def __call__(
        ...
        controlnet_weights: Optional[
            Union[float, list, np.ndarray, torch.FloatTensor]
        ] = 1.0,
        ...
        if trajectory_maps is not None:
            trajectory_maps = self.video_processor.preprocess_video(
                trajectory_maps, height=height, width=width
        ...
        # save_tensor_as_images_with_pca(latents, "visualization/duck/latents")
        # save_tensor_as_images_with_pca(image_latents, "visualization/duck/image_latents")
        ...
        # save_tensor_as_images_with_pca(controlnet_latents, "visualization/duck/video_latents")
        if do_classifier_free_guidance:
            controlnet_latents = torch.cat(
                [controlnet_latents, controlnet_latents], dim=0
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                if isinstance(controlnet_states, (tuple, list)):
                    controlnet_states = [
                        x.to(dtype=self.transformer.dtype) for x in controlnet_states
                    ]
                else:
                    controlnet_states = controlnet_states.to(
